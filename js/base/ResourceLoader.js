...
export class ResourceLoader{
  ...
  //所有图片加载完成后
  onloaded(){
    //加载过程是一个异步操作,所以可以使用回调方式,也可以使用promise
    //下面的代码中不能使用function,否则this的指向会发生改变
    return new Promise(resolve=>{
      let count=0;//计数器,统计加载完成的数量
      //遍历map中所有的图片对象(map的值)
      for(let img of this.map.values()){
        img.onload=()=>{
          //加载完成,count自增
          count++;
          //加载完成的条件,count的值和map的个数相等
